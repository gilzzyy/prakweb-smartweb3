...
function setupEventListeners() {
    ...
    if (elements.copyAddressBtn) {
        elements.copyAddressBtn.addEventListener('click', () => {
            const addr = elements.addressDisplay.textContent || '';
            if (addr) copyToClipboard(addr, 'Alamat disalin ke clipboard');
        });
    }

    if (elements.helpBtn) {
        elements.helpBtn.addEventListener('click', () => {
            showToast('Klik Connect untuk menghubungkan MetaMask. Gunakan tombol Store untuk menyimpan angka.', 'info', 6000);
        });
    }
}
